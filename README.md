...
| **Delivery Batches** | Group products into a delivery run. The overview lists every batch. Inside a batch, each product is a card: move ▲ ▼ / drag to set order, a position number, a Delivered toggle, a specs block (address in bold • recipient, then product name, ticket, delivery instructions), and on the right a batch chip (reassign) plus an ✕ to drop it from the batch. Materials aren't shown here. **Open Route in Maps** builds one Google Maps multi-stop directions link with every address in the current order. |
